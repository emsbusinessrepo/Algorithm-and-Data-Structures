    ...
    for day in range(len(revenue)):
        ...
        for city in range(len(revenue[-1])):
            if day_gain[city] != float('inf'):
                ...
                print(day_gain)
        bellman_ford_algorithm(travel_days, day_gain, day, yesterdays_gain)
...
def bellman_ford_algorithm(travel_days, day_gain, day, yesterdays_gain):
    for city_x in range(len(day_gain)):
        if day_gain[city_x] != 0:
            for city_y in range(len(travel_days[city_x])):
                ...
                if current_total_days != -1 and current_total_days <= next_day:
                    if day_gain[city_y] == float('-inf'):
                        day_gain[city_y] = 0
                    elif day_gain[city_y] < yesterdays_gain[current_total_days - next_day][city_x]:
                        day_gain[city_y] = yesterdays_gain[current_total_days - next_day][city_y]
